...
function applyPatch() {
  ...
  if (eyebrow) eyebrow.textContent = 'ΑΕΜ ΑΣΠΡΟΠΥΡΓΟΥ • ΣΧΟΛΗ ΜΗΧΑΝΙΚΩΝ'
  if (brand) brand.textContent = 'Παρουσιολόγια'
  if (title) title.textContent = 'Παρουσιολόγια Εργαστηρίων – Τεχνουργείων'
  if (copy) copy.textContent = 'Κεντρικό περιβάλλον για σπουδαστές, ομάδες, μαθήματα, καθηγητές, πρόγραμμα και παρουσιολόγια εργαστηρίων – τεχνουργείων.'
...
window.addEventListener('change', (event) => {
  if (event.target?.matches?.('select[aria-label="Επιλογή ακαδημαϊκής περιόδου"]')) {
