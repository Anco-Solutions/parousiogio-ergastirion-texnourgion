...
(function () {
  const installNavFix = () => {
    ...
    if (!document.getElementById('aen-stable-horizontal-nav')) {
      ...
      style.textContent = `
        .sidebar { display:block!important; width:100%!important; max-width:100%!important; min-width:0!important; box-sizing:border-box!important; overflow-x:auto!important; overflow-y:hidden!important; white-space:nowrap!important; touch-action:pan-x!important; -webkit-overflow-scrolling:touch!important; overscroll-behavior-x:contain!important; overscroll-behavior-y:none!important; scroll-behavior:auto!important; scroll-snap-type:none!important; scrollbar-width:none!important; padding-left:12px!important; padding-right:12px!important; }
        .sidebar::-webkit-scrollbar { display:none!important; width:0!important; height:0!important; }
        .sidebar > * { display:inline-flex!important; vertical-align:middle!important; flex:none!important; width:auto!important; min-width:0!important; max-width:none!important; box-sizing:border-box!important; }
        .sidebar > .nav-label { display:none!important; }
        .sidebar > .nav-item { width:auto!important; min-width:fit-content!important; max-width:none!important; flex:none!important; white-space:nowrap!important; margin-right:4px!important; }
      `
      document.head.appendChild(style)
    }
    if (!nav.dataset.aenNavFixed) {
      nav.dataset.aenNavFixed = '1'
      const refresh = () => { if (nav.isConnected) nav.style.setProperty('overflow-x','auto','important') }
  ...
  const apply = () => {
    ...
    if (!document.getElementById(styleId)) {
      ...
      style.id=styleId
      style.textContent=`
        .topbar .aen-school-primary,.topbar .aen-school-secondary{display:inline!important;white-space:nowrap!important}
        .topbar .aen-school-primary::after{content:' • '}
        .topbar .aen-school-primary,.topbar .aen-school-secondary{margin:0!important}
      `
      document.head.appendChild(style)
    }
    installNavFix()
  ...
  let tries=0
  const timer=setInterval(()=>{ if(apply() || ++tries>80) clearInterval(timer) },100)
  apply()
})()
